function ERR = L2_Error( U,uee,X,T,Ngauss,elem,k )
%L2_Error calcule l'errer dans l'espace L2
...
s = Xgauss(:,1); 
t = Xgauss(:,2); 
                            %les coefficient abcdef tel que x=a*s²+b*t²+c*st+d*s+e*t+f
...
J=[(2*abcdef(1,1)*s+abcdef(3,1)*t+abcdef(4,1)),(2*abcdef(2,1)*t+abcdef(3,1)*s+abcdef(5,1));
 ...
 for i=1:size(T,2)
     interm=interm+phi(j,i)*(U(T(ie,i))-uee(T(ie,i)));
 end
end
ERR=ERR+(interm^2)*abs(det(J([j j+Ngauss],:)))*Wgauss(j);
end
ERR=sqrt(ERR);
end
